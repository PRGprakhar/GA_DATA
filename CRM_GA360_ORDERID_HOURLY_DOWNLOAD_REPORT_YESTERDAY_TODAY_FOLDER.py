...
table_id = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','table_id')
#val_start_date =  configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_start_date')
val_start_date=datetime.datetime.now()- datetime.timedelta(days=1)
#val_end_date = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_start_date')
val_end_date=datetime.datetime.now()- datetime.timedelta(days=1)
val_metrics = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_metrics')
val_dimensions = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_dimensions')
val_sort = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_sort')
#val_filters = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_filters')
val_start_index = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_start_index')
val_max_results = configParser.get('CRM_GA360_ORDERID_HOURLY_REPORT','val_max_results')
...
def main(argv):
    ...
    try:
        ...
        if not os.path.exists(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_ORDERID_HOURLY')+  format(DATE)+  '/' +format(DATETIME)):
            os.makedirs(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_ORDERID_HOURLY')+  format(DATE)+  '/' +format(DATETIME))    
        df_result.to_csv(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+ configParser.get('GA360','CRM_FILE_PREFIX_ORDERID_HOURLY')+  format(DATE) +  '/' +format(DATETIME)+ '/' + configParser.get('GA360','CRM_FILE_PREFIX_ORDERID_HOURLY')+  format(DATETIME)+'.csv', index = None, header=True)
        #print(df_result)
...
LOCAL_FOLDER = configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_ORDERID_HOURLY') + format(DATE) + '/' +format(DATETIME)
BUCKET_FOLDER = 'ga360_orderid_hourly' + '/' + format(DATE)
TOTAL_CSV_ROWS = 0
