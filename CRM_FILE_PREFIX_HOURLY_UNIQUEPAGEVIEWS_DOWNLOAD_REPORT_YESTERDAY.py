...
table_id = configParser.get('CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS','table_id')
#val_start_date =  configParser.get('CRM_GA360_SKU_SESSIONS_REPORT','val_start_date')
val_start_date=datetime.datetime.now()- datetime.timedelta(days=1)
#val_end_date = configParser.get('CRM_GA360_SKU_SESSIONS_REPORT','val_start_date')
val_end_date=datetime.datetime.now()- datetime.timedelta(days=1)
val_metrics = configParser.get('CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS','val_metrics')
val_dimensions = configParser.get('CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS','val_dimensions')
#val_sort = configParser.get('CRM_GA360_SKU_SESSIONS_REPORT','val_sort')
#val_filters = configParser.get('CRM_GA360_SKU_SESSIONS_REPORT','val_filters')
val_start_index = configParser.get('CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS','val_start_index')
val_max_results = configParser.get('CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS','val_max_results')
...
def main(argv):
    ...
    user_info = service.management().accounts().list().execute()
    user_email = user_info['username']    
    print(f"The authenticated user's email address is: {user_email}")
    ...
    try:
        ...
        if not os.path.exists(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS')+  format(formatted_date)+  '/' +format(formatted_datetime)):
            os.makedirs(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS')+  format(formatted_date)+  '/' +format(formatted_datetime))    
        df_result.to_csv(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+ configParser.get('GA360','CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS')+  format(formatted_date) +  '/' +format(formatted_datetime)+ '/'+ configParser.get('GA360','CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS')+  format(formatted_datetime)+'.csv', index = None, header=True)
        #print(df_result)
...
#client = storage.Client.from_service_account_json('samsunguk-media-support-dcb883668e19.json')
PROJECT_ID = 'samsunguk-media-support'
BUCKET_STORAGE_NAME = 'ga360_hourly_report_data'
LOCAL_FOLDER = configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_HOURLY_UNIQUEPAGEVIEWS') + format(formatted_date) + '/' +format(formatted_datetime)
BUCKET_FOLDER = 'ga360_channel_pageviews' + '/' + format(formatted_date)
TOTAL_CSV_ROWS = 0
