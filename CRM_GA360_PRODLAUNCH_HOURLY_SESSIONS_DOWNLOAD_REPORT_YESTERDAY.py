...
table_id = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','table_id')
#val_start_date =  configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_start_date')
val_start_date=datetime.datetime.now()- datetime.timedelta(days=1) 
#val_end_date = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_start_date')
val_end_date=datetime.datetime.now() - datetime.timedelta(days=1)  #datetime.datetime.strptime('20221212', '%Y%m%d') #
val_metrics = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_metrics')
val_dimensions = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_dimensions')
val_sort = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_sort')
val_filters = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_filters')
val_start_index = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_start_index')
val_max_results = configParser.get('CRM_GA360_PRODLAUNCH_HOURLY_SESSIONS_REPORT','val_max_results')
...
def main(argv):
    ...
    try:
        ...
        if not os.path.exists(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_PRODLAUNCH_HOURLY_SESSIONS')+  format(formatted_date)+  '/' +format(formatted_datetime)):
            os.makedirs(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_PRODLAUNCH_HOURLY_SESSIONS')+  format(formatted_date)+  '/' +format(formatted_datetime))    
        df_result.to_csv(configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+ configParser.get('GA360','CRM_FILE_PREFIX_PRODLAUNCH_HOURLY_SESSIONS')+  format(formatted_date) +  '/' +format(formatted_datetime)+ '/' + configParser.get('GA360','CRM_FILE_PREFIX_PRODLAUNCH_HOURLY_SESSIONS')+  format(formatted_datetime)+'.csv', index = None, header=True)
        #print(df_result)
...
LOCAL_FOLDER = configParser.get('GA360','LOCAL_FOLDER_PATH_TO_REPORT')+configParser.get('GA360','CRM_FILE_PREFIX_PRODLAUNCH_HOURLY_SESSIONS') + format(formatted_date) + '/' +format(formatted_datetime)
BUCKET_FOLDER = 'ga360_prodlaunch_hourly_sessions' + '/' + format(formatted_date)
TOTAL_CSV_ROWS = 0
